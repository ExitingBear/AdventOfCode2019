...
## Part 1
Read the input in to a nice matrix (will read into a dictionary inside the function)

```{r}
#vaultmap<-matrix(nrow=0,ncol=nchar(input[1]))
#for(i in 1:length(input)){
#  x<-unlist(str_split(input[i],""))
#  vaultmap<-rbind(vaultmap,x)}
#strt<-which(vaultmap=="@",arr.ind=TRUE)
#strt
```
...
mrkfinder<-function(input){
  ...
  while(pq$size()>0){
    ...
    ### get some data
    if(nkl<keysunfound){
      cat("keys left: ",nkl," round: ",roundcount," steps: ",currs," queue: ",pq$size()," keys: ", currk,"\n")
      keysunfound<-nkl}
...
input<-read_lines("../../AoCData/AOC2019/Day18Part2.txt")
